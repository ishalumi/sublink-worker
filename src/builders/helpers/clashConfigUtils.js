import { REJECT_ACTION_RULES } from '../../config/index.js';

const resolveClashTarget = (outbound, translator) => {
    if (REJECT_ACTION_RULES.has(outbound)) return 'REJECT';
    return translator('outboundNames.' + outbound);
};
    ...
    rules
        ...
        .forEach(rule => {
            rule.src_ip_cidr.forEach(cidr => {
                if (!cidr) return;
                results.push(`SRC-IP-CIDR,${cidr},${resolveClashTarget(rule.outbound, translator)}`);
            });
        });
    rules
        ...
        .forEach(rule => {
            rule.domain_suffix.forEach(suffix => {
                results.push(`DOMAIN-SUFFIX,${suffix},${resolveClashTarget(rule.outbound, translator)}`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.domain_keyword.forEach(keyword => {
                results.push(`DOMAIN-KEYWORD,${keyword},${resolveClashTarget(rule.outbound, translator)}`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.site_rules.forEach(site => {
                results.push(`RULE-SET,${site},${resolveClashTarget(rule.outbound, translator)}`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.ip_rules.forEach(ip => {
                results.push(`RULE-SET,${ip}-ip,${resolveClashTarget(rule.outbound, translator)},no-resolve`);
            });
        });
    ...
    rules
        ...
        .forEach(rule => {
            rule.ip_cidr.forEach(cidr => {
                results.push(`IP-CIDR,${cidr},${resolveClashTarget(rule.outbound, translator)},no-resolve`);
            });
        });
